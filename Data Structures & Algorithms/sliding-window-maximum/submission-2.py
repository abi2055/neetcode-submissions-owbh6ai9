class Solution:
    def maxSlidingWindow(self, nums: List[int], k: int) -> List[int]:
        ...
        for index, value in enumerate(nums):
            heapq.heappush(max_store, (-value, index))
            # storing both the index of that value and the max of that value
            # python sorts by the first index 
            
            # index = right pointer
            if index >= k - 1:
                left_pointer = index - k + 1

                while max_store[0][1] < left_pointer:
                    heapq.heappop(max_store)

                results.append(-max_store[0][0])
